// YouTube-themed Cursor Fluid Animation - Working Implementation
...
function pointerPrototype() {
    ...
    this.down = false;
    this.moved = false;
    this.color = [255, 0, 0]; // YouTube red
}
...
// Main initializer - exact logic adapted from cursor_fluid.html
(function initYouTubeFluid() {
    document.addEventListener('DOMContentLoaded', () => {
        try {
            // Prepare canvas behind UI
            ...
            // Make variables from template scope
            window.__yt_fluid_ctx = { gl, ext, canvas };

            // Begin port of core from template
            ...
            function createProgram(vertexShader, fragmentShader) {
                ...
                // Ensure attribute location 0 is bound to aPosition
            ...
            function getResolution(resolution) {
                let aspectRatio = gl.drawingBufferWidth / gl.drawingBufferHeight; if (aspectRatio < 1) aspectRatio = 1.0 / aspectRatio; let min = Math.round(resolution); let max = Math.round(resolution * aspectRatio); return gl.drawingBufferWidth > gl.drawingBufferHeight ? {width:max,height:min}:{width:min,height:max};
            }
            ...
            // Additional shader sources from template
            ...
            const splatProgram = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, splatShaderSource));
            const advectionProgram = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, advectionShaderSource));
            const copyProgram            = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, copyShaderSource));
            const clearProgram           = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, clearShaderSource));
            const colorProgram           = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, colorShaderSource));
            const divergenceProgram      = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, divergenceShaderSource));
            const curlProgram            = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, curlShaderSource));
            const vorticityProgram       = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, vorticityShaderSource));
            const pressureProgram        = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, pressureShaderSource));
            const gradienSubtractProgram = new Program(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, gradientSubtractShaderSource));

            // Ensure display material has a compiled program
            ...
            // Pointers
            pointers = []; pointers.push(new pointerPrototype());
            ...
            function splat(x,y,dx,dy,color){
                // velocity
                advectionProgram.bind(); // ensure VAO / attrib setup
                splatProgram.bind();
            ...
            function drawColor (target, color) { colorProgram.bind(); gl.uniform4f(colorProgram.uniforms.color, color.r, color.g, color.b, 1.0); blit(target); }
            ...
            function update(){ const dt = calcDeltaTime(); if (resizeCanvas()) initFramebuffers(); updateColors(dt); applyInputs(); if (!config.PAUSED) step(dt); render(null); requestAnimationFrame(update); }
            update();

            console.log('YouTube fluid cursor animation initialized');
        } catch (e) {
            console.error('Failed to initialize fluid animation from template code:', e);
        }
    });
})();
